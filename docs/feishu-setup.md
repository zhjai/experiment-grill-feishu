...
2. An **inbound-triggered or heartbeat model turn** emits any unposted questions **as its turn output in the user's conversation** (OpenClaw's outbound adapter delivers that to Feishu — verified: `send.ts` calls `im.message.create`/`reply`), and writes the user's chat reply back into that sink.
3. The job picks the reply up from the sink at its next checkpoint.

**Reality check on OpenClaw's send.** OpenClaw genuinely sends to Feishu, but only through its **channel reply path** — the agent's turn output in a conversation is what gets delivered. There is **no `openclaw message send` CLI and no agent send-tool** (the only agent-exposed Feishu tools are docs/bitable/wiki/drive/perm). So 1C is not "call OpenClaw to push a message"; it's "an OpenClaw turn, running in the user's conversation, says the question." That needs the heartbeat/inbound turn to actually fire in that conversation — so it's a **Tier 1 / Tier 3 hybrid, not "free,"** and if you can't guarantee that turn fires, prefer Tier 2 (larksuite/cli, which *does* expose a real proactive send) for a detached OpenClaw job. OpenClaw Feishu docs: [docs.openclaw.ai](https://docs.openclaw.ai/zh-CN/channels/feishu) · [Lark community wiki](https://larkcommunity.feishu.cn/wiki/LDmXwEVhJitBa5kU0mjc16VKneb) · [plugin guide](https://www.feishu.cn/content/article/7613711414611463386).
